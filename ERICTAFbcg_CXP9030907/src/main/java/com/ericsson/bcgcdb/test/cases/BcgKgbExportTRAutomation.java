...
public class BcgKgbExportTRAutomation extends TorTestCaseHelper implements TestCase{
	Logger LOGGER = Logger.getLogger(BcgKgbExport.class);

	@Inject
	private OperatorRegistry<BCGKGBTRExportOperator> bcgExportProvider;
	 ...
	 * @PRIORITY HIGH
	 */
	@TestId(id = "OSS_76078_BCG_KGB", title = "BCG KGB Export TR Automatiuon")
	@Context(context = {Context.CLI})
	@Test(groups={"KGB","CDB"})
	@DataDriven(name = "bcg_KGB_export_tr_Automation")
...
//		bcgExportTestData.setTestID(TestID);
		...
		BCGKGBExportTrAutomationCliOperator bCGKGBExportOperator = (BCGKGBExportTrAutomationCliOperator) bcgExportProvider.provide(BCGKGBTRExportOperator.class);
