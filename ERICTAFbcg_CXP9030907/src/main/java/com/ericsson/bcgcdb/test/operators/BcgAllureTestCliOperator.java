...
public class BcgAllureTestCliOperator implements BCGAllureOperator {

	Logger LOGGER = Logger.getLogger(BcgAllureTestCliOperator.class);
...
//	Host host;
	...
	private static final Host host = HostGroup.getOssmaster();

    private static final CLICommandHelper helper = new CLICommandHelper(host);

    final CLI cli = new CLI(host);

//    CSTestHandler<CSDatabase.Segment> cshandler = new CSTestHandler<>(host, new CSDatabase.Segment());
...
//////		List<Host> netsimServers = HostGroup.getAllNetsims();
