...
public class ImportStkpi extends TorTestCaseHelper implements TestCase {
	 ...
	 * @PRIORITY HIGH
	 */
	@TestId(id="OSS-48697_STKPI_Import_1", title = "BCG STKPI Import Testcases For Deletion Of 1000 UtranRelations .")
	@Context(context = {Context.CLI})
	@Test(groups={"STCDB","KGB","CDB"})
	@DataDriven(name = "ImportStkpi")
	public void bCGImport(
		...
		try{
			...
			for(int i=1 ; i<=iterations;i++){
				...
				String importCommandOutput = importStkpiOperator.performImport(importFileName,planName);
...
//			assertTrue("MO rate is less than the previous baseline ",finalMORate>=Double.parseDouble(baseLineMoRate));
			...
			importStkpiOperator.exitCleanUp(importFileName);
		}
	}
}
